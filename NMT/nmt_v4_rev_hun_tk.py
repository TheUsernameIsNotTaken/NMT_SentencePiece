...
# with Hungarian to English (HU-EN) translation,
...
print("Hungarian sentence: ", input_sentences[180])
print("English translation: ", output_sentences_inputs[180])
...
for i in input_sentences:
      hu_len.append(len(i.split()))

for i in output_sentences:
      en_len.append(len(i.split()))

length_df = pd.DataFrame({'english':hu_len, 'hungarian':en_len})
...
if (DO_TRAINING):
    ...
    spm.SentencePieceTrainer.train(f'--input=hun-eng/timar_eng.txt --model_prefix=m_eng --vocab_size={MAX_NUM_TOKENS}')
    print("Training HUN SentencePiece Model...")
    spm.SentencePieceTrainer.train(f'--input=hun-eng/timar_hun.txt --model_prefix=m_hun --vocab_size={MAX_NUM_TOKENS}')

# Load in pre-trained models
# makes segmenter instance and loads the model file (m_hu.model & m_en.model)
sp_hu = spm.SentencePieceProcessor()
sp_hu.load('m_hu.model')
sp_en = spm.SentencePieceProcessor()
sp_en.load('m_en.model')

# Encode hu: text => id
input_integer_seq = [(sp_hu.encode_as_ids(in_sentence)) for in_sentence in input_sentences]
#print(input_integer_seq)

word2idx_inputs = {}
with open('m_hu.vocab', mode='r', encoding='utf-8') as in_file:
    i = 0
    for line in in_file:
...
# Encode hun: text => id
output_integer_seq = [(sp_en.encode_as_ids(in_sentence) + [2]) for in_sentence in output_sentences]
output_input_integer_seq = [([1] + sp_en.encode_as_ids(in_sentence)) for in_sentence in output_sentences_inputs]
#print(output_input_integer_seq)

word2idx_outputs = {}
with open('m_en.vocab', mode='r', encoding='utf-8') as in_file:
    i = 0
    for line in in_file:
        key = line.rstrip().split('\t')[0]
...
#print(sp_hu.decode_ids([1, 292, 27, 81, 3, 2]))
...
#print(sp_en.decode_ids([1, 826, 87, 78, 254, 4, 2]))
...
num_words = min(MAX_NUM_TOKENS, len(word2idx_inputs) + 1)
...
plot_model(model, to_file='model_plot4a_hu.png', show_shapes=True, show_layer_names=True)
...
if(DO_TRAINING):
    ...
    model.save('seq2seq_eng-hun_spt_rev.keras')
...
model.load_weights('seq2seq_eng-hun_spt_rev.keras')
...
from keras.utils import plot_model
plot_model(decoder_model, to_file='model_plot_dec_hu.png', show_shapes=True, show_layer_names=True)
...
while n < 40:
    ...
    transtaled_sentence = sp_en.decode(translation)
    print(f"--- #{n} ---")
    print('Input Language : ', input_sentences[i])
    print('Actual translation : ', output_sentences[i])
    print('English translation : ', transtaled_sentence)
    n += 1
